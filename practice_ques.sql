...
DELIMITER &&
create procedure select_pre_filter_1(IN var int)
BEGIN
	select * from bank_details where job = 'retired' and balance > var;
END &&

call select_pre_filter_1(200)

DELIMITER &&
create procedure select_pre_filter_2(IN var int, IN var1 varchar(30))
BEGIN
	select * from bank_details where job = var1 and balance > var;
END &&

call select_pre_filter_2(200,'technician')
